...
class Usuario {
    ...
    public function buscar($dados = array()){
        ...
        $query = "SELECT id, nome, login, foto, apelido, DATE_FORMAT(dataNascimento, '%d/%m/%Y') dataNascimento, telefone, email, capituloAtual, status
                  FROM usuarios
                  {$filtro}
    ...
    public function buscarPorID($id) {
        $query = "SELECT id, nome, login, foto, apelido, DATE_FORMAT(dataNascimento, '%d/%m/%Y') dataNascimento, telefone, email, capituloAtual, status FROM [tabela] WHERE id = ?";
        $dados = array($id);
        return $this->conexao->Buscar($query, $dados);
    }
